  ...
  const handleOpenModal = (client = null) => {
    if (client) {
      ...
      setFormData({
        ...
        currency:
          client.currency ||
          (client.amount && client.amount.includes('$') ? 'USD' : 'INR'),
    ...
    <div>
      ...
      >
        <div>
          ...
          >
            Manage your active 7-day projects and payments.
          </p>
        </div>
      ...
      <div className="table-container">
        {clients.length === 0 ? (
          ...
          >
            No clients available. When visitors sign up via chat, they will
            appear here.
          </div>
        ) : (
          <>
                ...
                <tbody>
                  {clients.map((client) => (
                    <tr key={client.id}>
                      ...
                      <td>
                        <span
                          className={`badge ${client.status === 'In Progress' ? 'warning' : client.status === 'New Lead' ? 'primary' : 'success'}`}
                        >
                          {client.status}
                        </span>
                      </td>
                      <td>
                        <span
                          className={`badge ${client.payment === 'Paid' ? 'success' : 'warning'}`}
                        >
                          {client.payment}
                        </span>
                      </td>
                ...
                >
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (formData.project) {
                        const srv = pricingServices.find(
                          (s) => s.title === formData.project
                        );
                        if (srv) {
                          if (newCurrency === 'USD')
                            newAmount = `$${srv.priceUS}`;
                          else if (newCurrency === 'INR')
                            newAmount = `₹${srv.priceIN}`;
                          else newAmount = '';
                        }
                      }
                      setFormData({
