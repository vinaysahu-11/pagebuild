...
const DashboardOverview = () => {
  ...
  const parseAmount = (amountStr, clientCurrency) => {
    ...
    const curr =
      clientCurrency ||
      (str.includes('$') || str.toUpperCase().includes('USD') ? 'USD' : 'INR');
  ...
  const convertTo = (value, fromCurr, toCurr) => {
    if (
      !exchangeRates ||
      fromCurr === toCurr ||
      !exchangeRates[fromCurr] ||
      !exchangeRates[toCurr]
    )
      return value;
  ...
  clients.forEach((c) => {
    if (c.deadline && c.payment === 'Paid') {
      const d = new Date(c.deadline).toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
      });
  ...
  const donutData = [
    {
      name: 'Completed',
      value: completedRevenue1,
      color: '#10b981',
    },
    {
      name: 'Pending',
      value: pendingPayments1,
      color: '#f59e0b',
    },
  ].filter((d) => d.value > 0);
  const serviceMap = {};
  ...
  const topServices = Object.values(serviceMap)
    ...
    .map((s) => ({
      ...
      percent:
        clients.length > 0 ? Math.round((s.orders / clients.length) * 100) : 0,
    }));
    ...
    >
      ...
      >
        <MetricCard
          ...
          icon={<Users size={20} color="var(--neon-purple)" />}
          percent=""
          graphColor="var(--neon-purple)"
        />
        <MetricCard
          title="Total Clients"
          value={clients.length}
          icon={<Briefcase size={20} color="var(--neon-blue)" />}
          percent=""
          graphColor="var(--neon-blue)"
        />
        <MetricCard
          title="Total Revenue"
          value={formatCurrency(totalRevenue1, primaryCurrency)}
          subValue={formatCurrency(totalRevenue2, secondaryCurrency)}
          icon={<DollarSign size={20} color="var(--success)" />}
          percent=""
          graphColor="var(--success)"
        />
        <MetricCard
          title="Pending Payments"
          value={formatCurrency(pendingPayments1, primaryCurrency)}
          subValue={formatCurrency(pendingPayments2, secondaryCurrency)}
          icon={<CreditCard size={20} color="var(--warning)" />}
          percent=""
          isNegative
          graphColor="var(--warning)"
        />
        <MetricCard
          title="Conversions"
          value={conversions}
          icon={<TrendingUp size={20} color="var(--secondary-color)" />}
          percent=""
          graphColor="var(--secondary-color)"
        />
      </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={revenueData}>
                  ...
                  <YAxis
                    ...
                    axisLine={false}
                    tickFormatter={(val) =>
                      formatCurrency(val, primaryCurrency)
                    }
                  />
                  <Tooltip
                    contentStyle={{
            ...
            {dbUsers.length === 0 && clients.length === 0 ? (
              ...
              <>
                <FunnelLayer
                  width="100%"
                  color="var(--neon-blue)"
                  label={`${dbUsers.length} Visitors`}
                  percent="100%"
                />
                <FunnelLayer
                  width="60%"
                  color="var(--warning)"
                  label={`${clients.length} Leads`}
                  percent={
                    dbUsers.length > 0
                      ? `${Math.round((clients.length / dbUsers.length) * 100)}%`
                      : '0%'
                  }
                />
                <FunnelLayer
                  ...
                  percent={
                    clients.length > 0
                      ? `${Math.round((conversions / clients.length) * 100)}%`
                      : '0%'
                  }
                />
              </>
        ...
        >
            ...
            {totalRevenue1 === 0 && pendingPayments1 === 0 ? (
              ...
              <>
                ...
                >
                  ...
                  >
                    {formatCurrency(
                      totalRevenue1 + pendingPayments1,
                      primaryCurrency
                    )}
                  </div>
                  <div
          ...
          >
            ...
            >
              ...
              >
                {formatCurrency(completedRevenue1, primaryCurrency)}
              </div>
            </div>
            <div
              style={{
            ...
            >
              ...
              >
                {formatCurrency(pendingPayments1, primaryCurrency)}
              </div>
            </div>
          </div>
        </div>
                ...
                {clients.length === 0 ? (
                  ...
                  clients.slice(0, 4).map((client, idx) => (
                    <tr
                      ...
                      style={{
                        borderBottom:
                          idx !== Math.min(clients.length - 1, 3)
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {client.status === 'Completed' ? '100%' : '60%'}
                          </span>
                        </div>
                      </td>
                    </tr>
...
const MetricCard = ({
  ...
  >
    ...
    >
      <path
        d="M0 20 Q 10 5, 20 15 T 40 10 T 60 15 T 80 5 T 100 15 L 100 20 L 0 20"
        fill={`url(#gradient-${title.replace(/\s+/g, '')})`}
        opacity="0.2"
      />
      <path
        d="M0 20 Q 10 5, 20 15 T 40 10 T 60 15 T 80 5 T 100 15"
        fill="none"
        stroke={graphColor}
        strokeWidth="2"
      />
